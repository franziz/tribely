...
- **After ANY SWE fix cycle that touches code, architecture-reviewer AND qa must re-run before advancing to Step 8.5 or Step 9.** No skipping the re-run because "the change looks small" or "tests passed locally." The orchestrator does not get to advance to manual smoke or PR creation directly off a SWE commit — both verification agents (`reviewer` and `qa`, re-engaged via `SendMessage`) must re-run and report clean (or surface findings that EL adjudicates) every cycle. This applies even to whitespace-only / format-only commits in code files — the cost of running them is small; the cost of shipping an unverified regression is high.
  ...
  - **Narrow exception — test-files-only commits.** If the SWE commit's changed-files set is entirely under `test/` or matches `*.test.ts` / `*_test.dart` / `*.spec.ts`, **reviewer** re-run is NOT required (reviewer scope explicitly excludes test files per `/api-review-architecture` + `/mobile-review-architecture` skill scope; reviewer will report "test files skipped per skill scope" every cycle). **qa re-run still applies** — the whole point of a test-data fix is to make qa pass, and qa is the verification. This exception is narrower than the pure-documentation one: it skips reviewer only, not qa. Verify with `git diff --name-only` before invoking; if any non-test file is present, the full re-run rule applies.
...
**Gate:** run this step ONLY if PM's step-2 brief flagged `needs-legal-compliance: true`. Otherwise emit `Step 4.5/11 — Legal & compliance  Agent: skipped  Verdict: no legal/compliance surface  Next: EL technical spec` and proceed to step 5.
...
Spawn `legal-compliance` into the team as `legal` with:
...
Step 4 (UI/UX design) is conditional — when skipped, emit `Step 4/11 — UI/UX design  Agent: skipped  Verdict: no UI/UX design surface  Next: EL technical spec`. Step 4.5 (Legal & compliance) is conditional — when skipped, emit `Step 4.5/11 — Legal & compliance  Agent: skipped  Verdict: no legal/compliance surface  Next: EL technical spec`. Step 8.5 (Manual on-device smoke) is conditional — when skipped, emit `Step 8.5/11 — Manual on-device smoke  Agent: skipped  Verdict: no hero-flow / time-validator surface  Next: PR creation`. Step 11 (Learning cycle + team teardown) always runs.
...
| Issue is a non-engineering ticket (no repo/code deliverable, no technical surface) | Refuse at step 1 BEFORE creating branch or team. Route to its owner: strategic → `ceo`; backlog/product shaping → `product-manager`; regulatory/policy with no repo artifact → `legal-compliance`. |
