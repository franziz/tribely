...
- **After ANY SWE fix cycle, architecture-reviewer AND qa must re-run before advancing to Step 8.5 or Step 9.** No skipping the re-run because "the change looks small" or "tests passed locally." The orchestrator does not get to advance to manual smoke or PR creation directly off a SWE commit — both verification agents must re-spawn and report clean (or surface findings that EL adjudicates) every cycle. This applies even to whitespace-only / format-only commits — the cost of running them is small; the cost of shipping an unverified regression is high.
