...
7. **Create the Agent Team** for this workflow: `TeamCreate({team_name: "tri-XX-<slug>", description: "Deliver TRI-XX end-to-end"})` (team name kebab-case, ≤64 chars). Every role agent in later steps is spawned into this team with a stable `name` and re-engaged via `SendMessage`. The team is torn down in Step 11.
   - **Harness fallback — implicit team.** If `TeamCreate`/`TeamDelete` are NOT available in the current harness (e.g. the `Agent` tool's `team_name` parameter is deprecated as "the session has a single implicit team", and a `select:TeamCreate,TeamDelete` tool search returns nothing), there is nothing to create — the team is implicit. Skip `TeamCreate`, spawn each role agent as a named background `Agent` (`run_in_background: true`, stable `name`), and re-engage via `SendMessage({to: "<name>"})` exactly as below. Teardown in Step 11 is then per-agent `shutdown_request` with NO `TeamDelete` call. The entire rest of the choreography — named teammates, `SendMessage` re-engagement, deliverable-before-idle contract — is identical. (Precedent: TRI-303.)
...
1. **Hero-flow path signals.** Run `git diff --name-only main...HEAD` and match changed files against:
   ...
   - **(c) browse + request to join:** `apps/mobile/lib/src/features/discover/presentation/**/event_detail*` OR `apps/mobile/lib/src/features/discover/presentation/**/discover_*` OR `apps/mobile/lib/src/features/join_requests/**/presentation/**` — the event-detail page and the browse surface live under **`discover/`**, NOT `events/` (the `events/` feature owns create-event; `discover/` owns browse + detail). A strict glob aimed at `events/.../event_detail*` or `events/.../browse*` matches nothing in the current tree and would skip smoke on a genuine hero-flow surface — the same `events/`→`discover/` path drift that also stales the ticket-prose file paths. Precedent: TRI-294 (post-auth capabilities re-read on the `discover/` event-detail resume-join path).
...
2. **Tear down the team.** After `/learn` completes: shut down each teammate via `SendMessage({to: "<name>", message: {type: "shutdown_request"}})`, wait for them to terminate, then call `TeamDelete`. (`TeamDelete` fails while members are still active, so shut down first.) The same teardown applies to a reused team on a `--resume` run, at the true end of the workflow.
   - **Harness fallback — implicit team.** If the harness has no `TeamDelete` (the implicit-team case from Step 1.7), the per-agent `shutdown_request` to each teammate IS the full teardown — there is no `TeamDelete` to call. Shut down every named teammate, confirm each terminated, and stop. (Precedent: TRI-303.)
