// Widget tests for RemoveAttendeeSheet.
//
// Covers:
//   1. Renders title with requester first name ("Remove {firstName}?").
//   2. Renders sub-head with event title.
//   3. Renders placeholder hint text in the field.
//   4. Submit button is disabled when input is empty.
//   5. Submit button is enabled after entering non-whitespace text.
//   6. Char counter shows "0 / 200" initially.
//   7. Char counter updates as user types.
//   8. Cancel with empty input pops without showing dialog.
...
import 'package:tribely/src/features/join_requests/presentation/widgets/remove_attendee_sheet.dart';
...
Future<void> _pumpSheet(
  ...
  String eventTitle = 'Morning Hike at Bukit Timah',
}) async {
...
void main() {
  group('RemoveAttendeeSheet', () {
    // -----------------------------------------------------------------------
    // 1. Title uses first name
    // -----------------------------------------------------------------------
    testWidgets('renders title with requester first name', (tester) async {
      await _pumpSheet(tester, onSubmit: (_) async => null);

      expect(find.textContaining('Remove Priya?'), findsOneWidget);
    });

    // -----------------------------------------------------------------------
    // 2. Sub-head contains event title
    // -----------------------------------------------------------------------
    testWidgets('renders sub-head with event title', (tester) async {
      await _pumpSheet(tester, onSubmit: (_) async => null);

      expect(
        find.textContaining('Morning Hike at Bukit Timah'),
        findsOneWidget,
      );
    });
    ...
    // 3. Placeholder hint
    // -----------------------------------------------------------------------
    testWidgets('renders placeholder hint text in the field', (tester) async {
      await _pumpSheet(tester, onSubmit: (_) async => null);

      expect(find.textContaining('Add a note for Priya'), findsOneWidget);
    });
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final button = tester.widget<PrimaryButton>(
          find.widgetWithText(PrimaryButton, 'Remove Priya'),
        );
                ...
                builder: (context) => ElevatedButton(
                  onPressed: () async {
                    await showRemoveAttendeeSheet(
                      context,
                      eventTitle: 'Morning Hike at Bukit Timah',
    ...
    testWidgets('Submit calls onSubmit with trimmed text', (tester) async {
      ...
      await tester.tap(find.widgetWithText(PrimaryButton, 'Remove Priya'));
      await tester.pumpAndSettle();
