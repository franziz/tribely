...
void main() {
  ...
  group('VenuePickerController', () {
    ...
    test(
      'selectSuggestion transitions to VenuePickerSelected on success',
      () async {
        final (:container, :port) = _makeContainer();
        addTearDown(container.dispose);
    ...
    test(
      'selectSuggestion is a no-op if already selected with the same place ID',
      () async {
        final (:container, :port) = _makeContainer();
        addTearDown(container.dispose);
    ...
    test('clearSelection resets to VenuePickerInitial', () async {
      ...
      final controller = container.read(venuePickerControllerProvider.notifier);
    ...
    test(
      'retrieve QuotaExhaustedFailure transitions to VenuePickerDegradedQuota',
      () async {
        final (:container, :port) = _makeContainer();
        addTearDown(container.dispose);
    ...
    test(
      'retrieve NetworkFailure transitions to VenuePickerDegradedNetwork',
      () async {
        final (:container, :port) = _makeContainer();
        addTearDown(container.dispose);
