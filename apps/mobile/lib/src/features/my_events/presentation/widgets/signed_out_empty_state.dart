...
import '../string_assets/my_events_signed_out_copy.dart';

/// Signed-out empty state for [MyEventsPage].
///
/// Shown when [MyEventsSignedOut] is the active controller state —
/// the session is unauthenticated and no fetch has fired. Presents
/// copy and a "Sign in" CTA that opens the sign-in gate sheet.
///
/// On successful sign-in the session flip auto-rebuilds the controller
/// into the authed path — no explicit navigation is needed here.
class SignedOutEmptyState extends ConsumerWidget {
  ...
  Future<void> _onSignInTapped(BuildContext context) async {
    ...
    // this controller into the authed path — no further action needed here.
