...
import type { PrunePostEventCheckInsResult } from '../../application/dto/prune-post-event-check-ins.result.js';
import type { PrunePostEventCheckInsUseCase } from '../../application/usecases/prune-post-event-check-ins.usecase.js';
import { PrunePostEventCheckInsJob } from './prune-post-event-check-ins.job.js';
 ...
 * PrunePostEventCheckInsUseCase` — the same pattern as the audit job tests.
 */
interface ExecutableUseCase {
  execute(): Promise<PrunePostEventCheckInsResult>;
}

class FakePruneUseCase implements ExecutableUseCase {
  readonly calls: CapturedExecuteCall[] = [];
  private result: PrunePostEventCheckInsResult = {
    pendingDeleted: 0,
    okDeleted: 0,
    flaggedResolvedDeleted: 0,
  };
  shouldThrow = false;

  setResult(r: PrunePostEventCheckInsResult): void {
    this.result = r;
  }
...
function buildJob(pruneUseCase: FakePruneUseCase, logger: FakeLogger): PrunePostEventCheckInsJob {
  return new PrunePostEventCheckInsJob({
    pruneUseCase: pruneUseCase as unknown as PrunePostEventCheckInsUseCase,
...
describe('PrunePostEventCheckInsJob', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it('tick is wrapped in runAsSystem — requestId has system:cron.prune-post-event-check-ins prefix', async () => {
    const useCase = new FakePruneUseCase();
    const logger = new FakeLogger();
    const job = buildJob(useCase, logger);
    ...
    expect(call?.requestId).toMatch(/^system:cron\.prune-post-event-check-ins:/);

    await job.stop();
  });

  it('logs INFO with all three bucket counts on success', async () => {
    const useCase = new FakePruneUseCase();
    const logger = new FakeLogger();
    useCase.setResult({ pendingDeleted: 3, okDeleted: 7, flaggedResolvedDeleted: 1 });
    ...
    const infoLine = logger.infoLines.find((l) =>
      l.message.includes('Post-event check-in retention sweep complete'),
    );
    expect(infoLine).toBeDefined();
    expect(infoLine?.payload).toMatchObject({
      pendingDeleted: 3,
      okDeleted: 7,
      flaggedResolvedDeleted: 1,
    });
