...
const valid = { address: '1 Raffles Place, Singapore', latitude: 1.2841, longitude: 103.8511 };
...
describe('Venue', () => {
  describe('create', () => {
    ...
    it('accepts the global boundary values', () => {
      expect(() => Venue.create({ address: 'pole', latitude: -90, longitude: -180 })).not.toThrow();
      expect(() => Venue.create({ address: 'pole', latitude: 90, longitude: 180 })).not.toThrow();
    });
  });
