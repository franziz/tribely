...
describe('detectPrivateVenue', () => {
  ...
  describe('private category → category_not_public', () => {
    ...
    it.each(privateValues)('returns category_not_public for private category "%s"', (value) => {
      const result = detectPrivateVenue({
        category: VenueCategory.create(value),
  ...
  describe('keyword matching — case-insensitive substring', () => {
    it.each(PRIVATE_KEYWORDS)('matches keyword "%s" in lowercase', (keyword) => {
      const result = detectPrivateVenue({
    ...
    it.each(PRIVATE_KEYWORDS)('matches keyword "%s" in UPPERCASE', (keyword) => {
      const result = detectPrivateVenue({
        category: VenueCategory.create('park'),
    ...
    it.each(PRIVATE_KEYWORDS)('matches keyword "%s" in mixed case', (keyword) => {
