...
export interface PrunePostEventCheckInEventsJobDeps {
...
/**
 * Scheduled job that calls PrunePostEventCheckInEventsUseCase on a fixed
 * interval. Driven by the PDPA s25 retention policy: audit rows whose
 * `occurredAt` exceeds 24 calendar months must be pruned.
 *
 * Design choices:
 ...
 *   persists `requestId=system:cron.prune-post-event-check-in-events:<cuid>` for
 ...
 */
export class PrunePostEventCheckInEventsJob {
  private readonly pruneUseCase: PrunePostEventCheckInEventsUseCase;
  ...
  constructor(deps: PrunePostEventCheckInEventsJobDeps) {
  ...
  private async runTick(): Promise<void> {
    ...
    try {
      await runAsSystem('cron.prune-post-event-check-in-events', async () => {
        ...
        this.logger.info(
          ...
          'Post-event check-in audit sweep complete',
        );
      });
    } catch (err) {
      this.logger.warn({ err }, 'Post-event check-in audit sweep failed');
    } finally {
